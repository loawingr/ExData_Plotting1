...
## convert the factor columns to numeric format so it can be plotted
voltage <- as.numeric(as.character(feb1to2$Voltage))
globalActivePower <- as.numeric(as.character(feb1to2$Global_active_power))
globalReactivePower <- as.numeric(as.character(feb1to2$Global_reactive_power))
...
## create a 2x2 matrix for the 4 plots to live in
par(mfcol = c(2,2))

plot(feb1to2$Time, globalActivePower, 
     type="l", 
...
plot(feb1to2$Time, voltage,
     type="l", 
     ylab = "Voltage", 
     xlab = "datetime" )

plot(feb1to2$Time, globalReactivePower,
     type="l", 
